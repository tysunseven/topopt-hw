...
- Based on the comparison of the four cases above, it is evident that the direction of the force does not affect the final optimization result. In fact, from the perspective of theoretical derivation,
$KU = F$
 is a linear equation. When $F$ changes to $-F$, $U$ changes to $-U$. However, the objective function $c$ is a quadratic function of $U$, so changing $U$ to $-U$ does not affect the value of $c$, and thus does not influence the update process of $x$ at each step.
