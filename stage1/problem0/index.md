...
for elx = 1:nelx
  for ely = 1:nely
    ...
    n2 = (nely+1)* elx   +ely;
```
- 这里的 n1 和 n2 分别是该单元左下角和右下角节点的全局编号。
- (nely+1) 是 x 方向上节点的数量，每列有 nely + 1 个节点。
- n1 对应于左下角节点，n2 对应于右下角节点，通过这些编号可以确定该单元的节点在全局刚度矩阵中的位置。

```matlab
    edof = [2*n1-1; 2*n1; 2*n2-1; 2*n2; 2*n2+1; 2*n2+2; 2*n1+1; 2*n1+2];
```
- edof 是一个向量，包含当前单元的 8 个自由度（DOFs）的全局索引。
